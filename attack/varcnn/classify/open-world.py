...
from metrics import *
from var_classifier import VarClassifier
...
def main(X, y, classes, m_file, load_model):
    ...
    if load_model: # load the trained model.
        clf = VarClassifier(classes, device, m_file) 
    else: # create & train a new model.   
        logger.info(f"run the training loop.") 
        clf = VarClassifier(classes, device)     
        clf.train(train_dl, valid_data)
        torch.save({'model_dir': clf.model_dir.state_dict(),
                    'model_time': clf.model_time.state_dict(),
                    'threshold': clf.threshold}, m_file)
