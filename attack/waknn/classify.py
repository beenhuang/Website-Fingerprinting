...
from metrics import *
from wa_classifier import WaClassifier

def main(X, y, classes, m_file, load_model):
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=random.randint(0,10000), stratify=y)
    X_train, X_target, y_train, y_target = train_test_split(X_train, y_train, test_size=0.1, random_state=random.randint(0,10000), stratify=y_train)
    #X_train, X_others, y_train, y_others = train_test_split(X_train, y_train, test_size=0.9, random_state=random.randint(0,10000), stratify=y_train)
    print(f"X_train:{X_train.shape}, X_target:{X_target.shape}, X_test:{X_test.shape}")
    #print(f"labels:{Counter(y_target)}")
    
    clf = WaClassifier(100)  

    logger.info(f"run the training.")
    clf.train(X_train, y_train, X_target, y_target)
