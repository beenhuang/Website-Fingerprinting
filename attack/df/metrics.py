...
def df_openworld(y_true, y_pred, label_unmon=100):
    y_true_new = [np.argmax(x) for x in y_true]
    y_pred_new = [np.argmax(x) for x in y_pred]

    #print(list(zip(y_true_new, y_pred_new)))

    return openworld_score(y_true_new, y_pred_new, max(y_true_new))

def df_closedworld(y_true, y_pred):
    y_true_new = [np.argmax(x) for x in y_true]
    y_pred_new = [np.argmax(x) for x in y_pred]

    return closedworld_score(y_true_new, y_pred_new)
...
def closedworld_score(y_true, y_pred):
    ...
    unequal = [x for x in y_true_pred if x[0] != x[1]]
    sorted_unequal = sorted(unequal, key=lambda x:x[0]) 
