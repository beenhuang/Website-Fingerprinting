...
def pr_curve(label, score, label_unmon=0):
    ...
    #PrecisionRecallDisplay.from_predictions(label, score) 
    #plt.show()
    precision, recall, thres = precision_recall_curve(label, score, pos_label=1, drop_intermediate=False)
    #print(thres)
    #precision = precision[:-300]
    #recall = recall[:-300]
    ...
    #plt.plot(thres, color='b')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.show()

def roc_curve(y_true, y_score, label_unmon=0):
    # monitored set is 1, unmonitored set is 0.
    #y_true = [0 if x == label_unmon else 1 for x in y_true]    
...
def ow_score_twoclass(y_true, y_pred, label_unmon=0):
    ...
    for idx, label_pred in enumerate(y_pred): 
        ...
        if label_true != label_unmon: # positive sample
            if label_pred != label_unmon: # predict positive
                tp += 1
    ...
    #pred_incorrect = [x for x in y_true_pred if x[0] != x[1] and y_true]
