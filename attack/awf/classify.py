...
from metrics import *
from awf_classifier import AWFClassifier

def main(X, y, classes, m_file, load_model):
    # get train_dataloader & test_dataloader
    #train_dl, test_dl = train_test_dataloader(X, y) 
    train_dl, valid_dl, test_dl = train_valid_test_dataloader(X, y)  
    ...
    if load_model: # load the trained model
        clf = AWFClassifier(classes, device, m_file) 
    else: # create&train new model    
        clf = AWFClassifier(classes, device)  # create DFNet mode   

        logger.info(f"run the training loop.")
        #clf.train(30, train_dataloader)
        clf.train(8, train_dl, valid_dl)
        torch.save(clf.model, m_file)
        
    logger.info(f"run the test.")
    label, pred, score = clf.test(test_dl)
