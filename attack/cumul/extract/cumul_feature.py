...
def get_feature(trace, featureCount=100, packet_size=514, separateClassifier=False):
    sizes = [x[1]*packet_size*-1 for x in trace]
    ...
    # Process sizes
    for packetsize in itertools.islice(sizes, None): 
        # incoming packets
        if packetsize > 0:
            inSize += packetsize
            inCount += 1
