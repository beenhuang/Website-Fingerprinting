...
def preprocess2(trace):
    ...
    for e in trace:
        times.append(float(e.split("\t")[0]))
        sizes.append(int(e.split("\t")[1].strip("\n")))
...
def extract(data_dir, feature_dir, file, umon_label=100):
    ...
    # get feature vector
    feature = Huang_cf.cf_feature(good_trace)  
    #feature = Ngram.NgramExtract(sizes, 3)
    #feature = []
    #Burst.BurstFeature(times, sizes, feature)
...
if __name__ == "__main__":
    ...
    MAIN_FEATURE_DIR = join(BASE_DIR, "attack", "test", "feature")
