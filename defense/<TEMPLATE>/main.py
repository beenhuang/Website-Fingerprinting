...
from tamaraw import defend

# return [[time, direction] ... ]
def standard_trace(trace, packet_size=514):
    start_time = float(trace[0].split("\t")[0])
    
    std_trace = []
    for e in trace:
        e = e.split("\t")
        timestamp = float(e[0]) - start_time
        direction = int(e[1].strip("\n"))
        std_trace.append([timestamp, direction])
...
def simulate(data_dir, out_dir, file):
    ...
    std_trace = standard_trace(trace)  
    # get the defended trace
    defend_trace = defend(std_trace)
